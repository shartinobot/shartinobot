...
async def dice_bet_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"🎲 **انتخاب ضریب**\n\n"
        f"💰 مبلغ شرط: {amount:,} تومان\n\n"
        f"📌 یکی از ضرایب زیر را انتخاب کنید:\n\n"
        f"• **زوج**: مجموع ۲ تاس زوج باشد (ضریب ۲)\n"
        f"• **فرد**: مجموع ۲ تاس فرد باشد (ضریب ۲)\n"
        f"• **مجموع ۱۰ یا بیشتر**: مجموع ۲ تاس ۱۰ یا بیشتر باشد (ضریب ۳)\n"
        f"• **هر ۲ تاس یکسان**: هر ۲ تاس یک عدد باشند (ضریب ۵)",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def coin_predict(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_win:
        ...
        result_text += f"💰 مبلغ شرط: {bet_amount:,} تومان\n"
        result_text += f"📊 ضریب: ۲.۵\n"
        result_text += f"🏆 جایزه: {win_amount:,} تومان\n\n"
        result_text += f"💰 موجودی جدید: {user['balance']:,} تومان"
        add_transaction(user_id, win_amount, "win", "برد در شیر یا خط (ضریب ۲.۵)")
    else:
        user["balance"] -= bet_amount
        user["total_losses"] = user.get("total_losses", 0) + 1
        result_text = f"😔 **متاسفم... شما باختید.**\n\n"
...
async def football_predict(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_win:
        ...
        result_msg += f"📊 ضریب: ۲.۵\n"
        result_msg += f"🏆 جایزه: {win_amount:,} تومان\n\n"
        result_msg += f"💰 موجودی جدید: {user['balance']:,} تومان"
        add_transaction(user_id, win_amount, "win", "برد در فوتبال (ضریب ۲.۵)")
    else:
        user["balance"] -= bet_amount
        user["total_losses"] = user.get("total_losses", 0) + 1
